...
module.exports = function (regl) {
  ...
  const drawPolytope = regl({
    ...
    void main () {
      ...
      vcolor = hsl2rgb(
        hue,
        0.5 * pow(cos(position.z + 2. * time), 2.) + texture2D(freq, vec2(0.1)).r,
  ...
  const drawPolytopeEdges = regl({
    ...
    void main () {
      vcolor = hsl2rgb(hue, 0.8, luminance);
  ...
  function scene ({tick, pcmData, beats}) {
    ...
    polytopes.forEach((p) => {
      p.angle += 0.01
      p.position[1] -= 0.01
      if (beats[1] && Math.random() > 0.9) {
        p.hue += 0.05 * (Math.random() - 0.5)
      }
    })
    waves.forEach((w) => {
      w.angle += 0.1
